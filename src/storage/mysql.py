...
class MySQLLogger(BaseSQLLogger):
    ...
    def __init__(
        ...
        mysql_user="root",
        mysql_password="",
        mysql_database="DouK-Downloader",
        *args,
        **kwargs,
    ):
    ...
    async def _save(self, data, *args, **kwargs):
        columns = ", ".join(f"`{i}`" for i in self.title_line)
        placeholders = ", ".join("%s" for _ in self.title_line)
        updates = ", ".join(
            [f"`{i}`=VALUES(`{i}`)" for i in self.title_line]
        )
        insert_sql = (
            ...
            try:
                ...
                print(
                    Text(
                        " ".join(
                            (
                                _("更新数据表名称时发生错误，重命名失败，请向作者反馈以便修复问题！"),
                                str(e),
                                old_sheet,
                                new_sheet,
                            )
